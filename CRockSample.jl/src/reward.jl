...
function CPOMDPs.costs_limit(pomdp::RockSampleCPOMDP)
    return [pomdp.bad_rock_cost_limit]
end

CPOMDPs.n_costs(::RockSampleCPOMDP) = 1
CPOMDPs.min_reward(p::RockSampleCPOMDP) = p.step_penalty + min(p.bad_rock_penalty, p.sensor_use_penalty)
CPOMDPs.max_reward(p::RockSampleCPOMDP) = p.step_penalty + max(p.exit_reward, p.good_rock_reward)
