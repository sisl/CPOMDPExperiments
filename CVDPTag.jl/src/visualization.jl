@recipe function f(p::CVDPTagProblem)
    m = mdp(p)
    ratio --> :equal
    xlim --> (-5, 5)
    ylim --> (-5, 5)
...
@recipe function f(pomdp::CVDPTagPOMDP, h::POMDPTools.AbstractSimHistory)
    ratio --> :equal
    xlim --> (-5, 5)
    ylim --> (-5, 5)
    @series mdp(pomdp), h
...
@recipe function f(p::CVDPTagProblem, h::POMDPTools.AbstractSimHistory)
    m = mdp(p)
    ratio --> :equal
    xlim --> (-5, 5)
    ylim --> (-5, 5)
...
function Plots.quiver(p::CVDPTagProblem)
    m = mdp(p)
    lim = (-3.5,3.5)
