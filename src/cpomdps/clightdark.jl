...
function CLightDarkNew(;pomdp::P=LightDark1D(0.95, 100., -100., 1., -0., (x)->abs(x - 10) + 1e-2 ), # default 0 incorrect_r, goes into cost
    ...
    return CLightDarkNew{P, statetype(pomdp), actiontype(pomdp), obstype(pomdp)}(pomdp,bad_action_budget, lambda)
end

POMDPs.reward(p::CLightDarkNew, s::LightDark1DState, a::Int) = POMDPs.reward(p.pomdp, s, a) + p.λ⋅costs(p, s, a)
costs(::CLightDarkNew, s::LightDark1DState, a::Int) = Float64[s.y > 12]
costs_limit(p::CLightDarkNew) = [p.bad_action_budget]
